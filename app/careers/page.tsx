  ...
  return (
    <div className="bg-[#FDFBF7] pt-20 min-h-screen">
      ...
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto -mt-10 relative z-20">
        <AnimatedSection>
          <div className="w-full aspect-[21/9] rounded-3xl overflow-hidden img-zoom shadow-[0_30px_60px_rgba(0,0,0,0.6)] border border-black/5 relative">
            ...
            <div className="absolute bottom-10 left-10">
               <h2 className="text-4xl font-display font-bold text-[#1A1A1A] mb-2">Our Culture</h2>
               <p className="text-black/80 text-lg">Where passion meets perfection.</p>
            </div>
          </div>
        </AnimatedSection>
      </section>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {perks.map((p, i) => (
              <AnimatedSection key={p.title} delay={i * 0.1}>
                <div className="glass rounded-2xl p-8 border border-black/5 card-hover text-center h-full">
                  ...
                  <h3 className="text-[#1A1A1A] font-bold text-xl mb-3">{p.title}</h3>
      ...
      <section className="section-padding bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            {openPositions.map((pos, i) => (
              <AnimatedSection key={i} delay={i * 0.1}>
                <div className="glass rounded-2xl p-6 md:p-8 border border-black/5 hover:border-[#C8A97E]/30 transition-all duration-300 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 group">
                  <div>
                    <div className="flex gap-3 mb-3">
                      ...
                      <span className="px-3 py-1 rounded-full border border-black/5 text-black/50 text-[10px] uppercase tracking-widest font-bold">
                        {pos.type}
                      </span>
                    </div>
                    <h3 className="text-2xl font-display font-bold text-[#1A1A1A] group-hover:text-[#C8A97E] transition-colors">{pos.title}</h3>
                    <p className="text-black/40 text-sm mt-2 flex items-center gap-2"><MapPin className="w-3.5 h-3.5" /> {pos.location}</p>
                  </div>
          ...
          <AnimatedSection delay={0.4} className="mt-16 text-center">
             <p className="text-black/50 mb-4">Don't see a role that fits? We're always open to meeting talented people.</p>
             <a href={`mailto:${SITE_CONFIG.email}`} className="text-[#C8A97E] font-semibold hover:text-[#1A1A1A] transition-colors underline underline-offset-4 decoration-[#C8A97E]/30">
