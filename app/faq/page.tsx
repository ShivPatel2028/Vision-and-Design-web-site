    ...
    <div className="bg-[#16161a] pt-20 min-h-screen">
          ...
          <div className="space-y-4">
            {FAQS.map((faq, i) => (
              <AnimatedSection key={i} delay={i * 0.05}>
                <div className={`glass-dark rounded-2xl border transition-all duration-300 overflow-hidden ${openIdx === i ? 'border-[#C8A97E]/40 shadow-[0_0_20px_rgba(200,169,126,0.1)]' : 'border-white/5 hover:border-white/20'}`}>
                  <button
                    className="w-full flex items-center justify-between p-6 text-left group"
                    onClick={() => setOpenIdx(openIdx === i ? null : i)}
                  >
                    <span className={`font-semibold pr-4 transition-colors duration-300 ${openIdx === i ? 'text-[#C8A97E]' : 'text-white group-hover:text-[#C8A97E]'}`}>
                      {faq.question}
                    </span>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 ${openIdx === i ? 'bg-[#C8A97E] text-black' : 'bg-white/5 text-white'}`}>
                  ...
                  <AnimatePresence>
                    {openIdx === i && (
                      ...
                      >
                        <div className="px-6 pb-6 pt-0 text-white/60 leading-relaxed border-t border-white/5 pt-4 mt-2">
                          {faq.answer}
                        </div>
                      </motion.div>
                    )}
      ...
      <section className="py-24 bg-[#1a1a1f]">
        <div className="max-w-3xl mx-auto text-center px-4">
          <AnimatedSection>
            <h2 className="text-4xl font-display font-bold text-white mb-6">Still have <span className="text-gradient-gold">Questions?</span></h2>
            <p className="text-white/60 mb-10 text-lg">We are here to help. Reach out to us directly for any specific inquiries.</p>
