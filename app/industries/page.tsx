    ...
    <div className="bg-[#FDFBF7] pt-20">
            ...
            {INDUSTRIES.map((ind, i) => (
              <div key={ind.id} className={`flex flex-col lg:flex-row gap-16 items-center ${i % 2 !== 0 ? 'lg:flex-row-reverse' : ''}`}>
                <AnimatedSection direction={i % 2 === 0 ? 'left' : 'right'} className="w-full lg:w-1/2">
                  <div className="relative img-zoom rounded-2xl overflow-hidden aspect-[4/3] group border border-black/5">
                ...
                <AnimatedSection direction={i % 2 === 0 ? 'right' : 'left'} className="w-full lg:w-1/2">
                  ...
                  <h2 className="text-4xl md:text-5xl font-display font-bold text-[#1A1A1A] mb-6">{ind.name}</h2>
                  <p className="text-black/60 text-lg leading-relaxed mb-8">{ind.description}</p>
                  
                  <div className="mb-8">
                    <h4 className="text-[#1A1A1A] font-semibold mb-4 text-sm uppercase tracking-wider">Core Solutions</h4>
                    <div className="flex flex-wrap gap-3">
                      {ind.services.map(s => (
                        <span key={s} className="px-4 py-2 rounded-full glass border border-black/5 text-sm text-black/70">
                          {s}
                        </span>
                      ))}
                    </div>
                  ...
                  <Link href={`/portfolio`} className="inline-flex items-center gap-2 text-[#C8A97E] font-semibold hover:text-[#1A1A1A] transition-colors duration-300 group">
      ...
      <section className="py-24 bg-white">
        <div className="max-w-3xl mx-auto text-center px-4">
          <AnimatedSection>
            <h2 className="text-4xl font-display font-bold text-[#1A1A1A] mb-6">Is Your Industry <span className="text-gradient-gold">Not Listed?</span></h2>
            <p className="text-black/60 mb-10 text-lg">We thrive on new challenges. Let's discuss how our creative approach can revolutionize your specific market.</p>
