    ...
    <div className="bg-[#F2F0ED] pt-20">
      ...
      <section className="relative py-32 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <AnimatedSection>
            ...
            <h1 className="text-5xl md:text-7xl font-display font-bold text-[#1A1A1A] mb-6">
            ...
            {SERVICES.map((service, i) => {
              ...
              return (
                <AnimatedSection key={service.id} delay={i * 0.07}>
                  <div className="group glass rounded-2xl border border-black/5 hover:border-[#C8A97E]/20 overflow-hidden transition-all duration-500">
                    <div className={`grid lg:grid-cols-2 gap-0 ${i % 2 === 1 ? 'lg:grid-flow-col-dense' : ''}`}>
                      ...
                      <div className="p-8 lg:p-12 flex flex-col justify-center">
                        ...
                        <h2 className="text-[#1A1A1A] font-display font-bold text-3xl mb-4 group-hover:text-[#C8A97E] transition-colors duration-300">{service.title}</h2>
                        <p className="text-black/60 leading-relaxed mb-6">{service.description}</p>
                        <ul className="grid grid-cols-2 gap-2 mb-8">
                          {service.features.map((f) => (
                            <li key={f} className="flex items-center gap-2 text-sm text-black/50">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {process.map((p, i) => (
              <AnimatedSection key={p.step} delay={i * 0.1}>
                <div className="glass rounded-2xl p-6 border border-black/5 hover:border-[#C8A97E]/20 card-hover text-center group">
                  <div className="text-5xl font-display font-bold text-gradient-gold mb-4">{p.step}</div>
                  <h3 className="text-[#1A1A1A] font-bold text-lg mb-2 group-hover:text-[#C8A97E] transition-colors duration-300">{p.title}</h3>
                  <p className="text-black/50 text-sm">{p.description}</p>
                </div>
              </AnimatedSection>
            ))}
          </div>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center px-4">
          <AnimatedSection>
            <h2 className="text-4xl md:text-5xl font-display font-bold text-[#1A1A1A] mb-6">Ready to <span className="text-gradient-gold">Get Started?</span></h2>
            <p className="text-black/60 text-lg mb-8">Tell us about your project and we'll craft a tailored creative solution for you.</p>
